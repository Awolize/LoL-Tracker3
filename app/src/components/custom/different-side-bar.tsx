...
import { updateAllChallengeData } from "@/server/different-challenge-queries";
...
export const DifferentSideBar = ({ challenges, username, region }: DifferentSideBarProps) => {
							...
							return (
								<li
									...
									className={`relative cursor-pointer rounded-sm border py-2 duration-300 ${
										selectedChallengeId === item.config.id ? "bg-background" : ""
									}`}
								...
								>
									<div className="flex gap-x-2 px-2">
										...
										<div className="w-full">
											<p className="text-sm">{item.localization?.name || item.config.name}</p>
					...
					<div className="flex gap-2 mt-2">
						<Button
							...
							onClick={async () => {
								setIsUpdating(true);
								try {
									const result = await updateAllChallengeData({ data: { username, region } });
