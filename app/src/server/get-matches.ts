...
export async function getMatches(
	...
	const filteredMatches = matches.filter((match) => {
		...
		const hasUser = match.matchSummoners!.some((ms) => ms.summoner.puuid === user.puuid);
		const matchFilter =
			...
			(!gameStartTimestampGte || (matchInfo?.gameStartTimestamp || new Date(0)) >= gameStartTimestampGte);
