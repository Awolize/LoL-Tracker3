...
const MatchItem = ({ match }: { match: CompleteMatch }) => {
	...
	const { getChampionImage } = useDataDragonPath(`${match.MatchInfo.gameVersion.split(".").slice(0, 2).join(".")}.1`);
	...
	return (
		<Card className={`w-full ${isWin ? 'border-green-500/20 bg-green-50/10 dark:bg-green-950/10' : 'border-red-500/20 bg-red-50/10 dark:bg-red-950/10'}`}>
			<CardHeader className="pb-3">
				<div className="flex items-center">
					...
					<div className="hidden md:flex items-center gap-1 text-lg font-bold absolute left-1/2 transform -translate-x-1/2">
						<span className="text-green-600 dark:text-green-400">{userParticipant?.kills}</span>
						<span className="text-muted-foreground">/</span>
						<span className="text-red-600 dark:text-red-400">{userParticipant?.deaths}</span>
						<span className="text-muted-foreground">/</span>
						<span className="text-blue-600 dark:text-blue-400">{userParticipant?.assists}</span>
					</div>
					<div className="flex-1 text-right">
						<div className="text-sm text-muted-foreground">{match.MatchInfo?.gameMode}</div>
						<div className="text-sm text-muted-foreground">{match.MatchInfo?.gameType}</div>
					</div>
				</div>
				...
				<div className="space-y-4">
					...
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
						...
						<div className="space-y-2">
							<h4 className="font-semibold text-red-600 dark:text-red-400 text-sm">Red Team</h4>
							<MatchTable
						...
						{/* Team 100 (Blue Team) */}
						<div className="space-y-2">
							<h4 className="font-semibold text-blue-600 dark:text-blue-400 text-sm">Blue Team</h4>
							<MatchTable
								players={match.MatchInfo.participants as unknown as Array<any>}
								teamId={100}
								version={`${match.MatchInfo.gameVersion.split(".").slice(0, 2).join(".")}.1`}
