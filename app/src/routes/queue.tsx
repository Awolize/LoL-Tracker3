...
const createStats = (val: number): MetricStats => ({ avg: val, high: val, low: val });
  ...
  const addSummary = React.useCallback((summary: QueueDataSummary) => {
    setHistory((prev) => {
      ...
      const processSegment = (
        ...
        const segment = data.filter((h) => h.ts < cutoff);
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 p-6 md:p-8">
      <header className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white">Queue Dashboard</h1>
  ...
  return (
    <div className={clsx(
      ...
      <span className={clsx("relative flex h-2 w-2")}>
        ...
        <span className={clsx("relative inline-flex rounded-full h-2 w-2", active ? "bg-green-500" : "bg-red-500")}></span>
      </span>
...
const MetricChart = React.memo(({ data, metric, color }: MetricChartProps) => {
  ...
  return (
    <ResponsiveLine
      ...
      xScale={{ type: "time", format: "%Q", useUTC: false, precision: "second" }}
      xFormat="time:%H:%M:%S"
