...
const ChallengeChampionList = ({
	...
	const { showMasteryPoints, filterPoints, filterLevel, championsScale } =
	...
	const challenges = [
		{ id: 100201, name: "Heroes", description: "Master different heroes" },
		{
			id: 100301,
			name: "Ocean",
			description: "Master champions in ocean theme",
		},
		{
			id: 610003,
			name: "Adapt to All Situations",
			description: "Adapt to different playstyles",
		},
	];
	...
	const selectedMarked =
		(selectedChallengeId && manuallyMarked[profileId]?.[selectedChallengeId]) ||
		new Set<number>();
	...
	return (
		<div className="flex flex-col p-4">
			...
			{selectedChallengeId && (
				<div className="mb-4">
					<p>
						Marked Champions: {selectedMarked.size}/
						{champions.filter((c) => c.championPoints >= filterPoints).length}
					</p>
				</div>
			)}
