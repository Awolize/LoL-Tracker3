...
import { getSRMatches, getArenaMatches, getMatches } from "@/server/get-matches";
import type { ChampionDetails, Summoner } from "@/lib/types";
...
export const getJackOfAllChamps = createServerFn()
	...
	.handler(async ({ data }) => {
		...
		return (result?.challengeHeroes as any)?.map((ch: any) => ch.championDetail as ChampionDetails) ?? [];
	});
...
export const getChampionOcean = createServerFn()
	...
	.handler(async ({ data }) => {
		...
		return (result?.challengesChampionOceans as any)?.map((ch: any) => ch.championDetail as ChampionDetails) ?? [];
	});
...
export const getChampionOcean2024Split3 = createServerFn()
	...
	.handler(async ({ data }) => {
		...
		const result = await db.query.challenges.findFirst({
			where: eq(challenges.puuid, user.puuid),
			with: { challengesChampionOcean2024Split3s: { with: { championDetail: true } } },
		});

		return (result?.challengesChampionOcean2024Split3s as any)?.map((ch: any) => ch.championDetail as ChampionDetails) ?? [];
	});
...
export const getAdaptToAllSituations = createServerFn()
	...
	.handler(async ({ data }) => {
		...
		const result = await db.query.challenges.findFirst({
			where: eq(challenges.puuid, user.puuid),
			with: { challengesAdaptToAllSituations: { with: { championDetail: true } } },
		});

		return (result?.challengesAdaptToAllSituations as any)?.map((ch: any) => ch.championDetail as ChampionDetails) ?? [];
	});
...
export const getInvincible = createServerFn()
	...
	.handler(async ({ data }) => {
		...
		return (result?.challengesInvincibles as any)?.map((ch: any) => ch.championDetail as ChampionDetails) ?? [];
	});
...
export const updateJackOfAllChamps = createServerFn()
	...
	.handler(async ({ data }) => {
		...
		console.log(`${user.gameName}#${user.tagLine} (${user.region}) updated JackOfAllChamps with ${uniqueChampIds.length} champions`);

		return { success: true, message: `Updated JackOfAllChamps with ${uniqueChampIds.length} champions` };
	});
...
export const updateChampionOcean = createServerFn()
	...
	.handler(async ({ data }) => {
		...
		console.log(`${user.gameName}#${user.tagLine} (${user.region}) updated ChampionOcean with ${uniqueChampIds.length} champions`);

		return { success: true, message: `Updated ChampionOcean with ${uniqueChampIds.length} champions` };
	});
...
export const updateChampionOcean2024Split3 = createServerFn()
	...
	.handler(async ({ data }) => {
		...
		const matches = await getMatches(user, { gameStartTimestampGte: new Date("2024-09-18T00:00:00.000Z") });
		if (!matches) {
			return { success: false, message: "No matches found" };
		}
		...
		console.log(`${user.gameName}#${user.tagLine} (${user.region}) updated ChampionOcean2024Split3 with ${uniqueChampIds.length} champions`);

		return { success: true, message: `Updated ChampionOcean2024Split3 with ${uniqueChampIds.length} champions` };
	});
...
export const updateAdaptToAllSituations = createServerFn()
	...
	.handler(async ({ data }) => {
		...
		const participations = matches.flatMap((match) => {
			const participants = match.matchInfos?.[0]?.participants as any[] || [];
			return participants.filter((p) => p.puuid === user.puuid && p.placement === 1);
		});
		...
		console.log(`${user.gameName}#${user.tagLine} (${user.region}) updated AdaptToAllSituations with ${uniqueChampIds.length} champions`);

		return { success: true, message: `Updated AdaptToAllSituations with ${uniqueChampIds.length} champions` };
	});
...
export const updateInvincible = createServerFn()
	...
	.handler(async ({ data }) => {
		...
		const participations = matches.flatMap((match) => {
			const participants = match.matchInfos?.[0]?.participants as any[] || [];
			return participants.filter((p) => p.puuid === user.puuid && p.deaths === 0 && p.win);
		});
		...
		console.log(`${user.gameName}#${user.tagLine} (${user.region}) updated Invincible with ${uniqueChampIds.length} champions`);

		return { success: true, message: `Updated Invincible with ${uniqueChampIds.length} champions` };
	});

export const updateAllChallengeData = createServerFn()
	.inputValidator((input: { username: string; region: string }) => input)
	.handler(async ({ data }) => {
		console.log(`Updating all challenge data for user ${data.username} (${data.region.toUpperCase()})`);
